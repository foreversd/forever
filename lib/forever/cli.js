...
app.cmd(/set ([\w\-_]+) (.+)/, cli.set = function (key, value) {
  if (!!~['sockPath', 'logPath', 'pidPath'].indexOf(key)) {
    return forever.log.warn(key + ' is readonly.');
  }
  updateConfig(function () {
    forever.log.info('Setting forever config: ' + key.grey);
    //
    // Make sure Boolean and Number are their own types, but not String.
    //
    if (!isNaN(value)) {
    ...
    // reset directories if necessary.
    if (key == 'root') {
      ['pids', 'sock', 'logs'].forEach(function(k) {
        forever.config.set((k[k.length - 1] == 's' ? k.substr(0, k.length - 1) : k) + 'Path', path.join(forever.config.get('root'), k));
      });
    }
  });
});
