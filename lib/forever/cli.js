...
var getOptions = cli.getOptions = function (file) {
  ...
  var argumentOptions = process.argv.splice(process.argv.indexOf(file) + 1);
  ...
  // Initialize options with forever.json in the sourceDir
  var sourceDir = app.config.get('sourceDir') || (file && file[0] !== '/' ? process.cwd() : '/');

  var optsFilePath = sourceDir + '/forever.json';
  if(path.existsSync(optsFilePath)){
    options = JSON.parse(fs.readFileSync(optsFilePath, 'utf-8'));
  }
  
  options.options = argumentOptions;

  // Extend options with cli options
  [
