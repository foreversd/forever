...
function getSockets(sockPath, callback) {
  ...
  try {
    ...
    //
    // Although `readdirSync` always sort files by name, but we must make sure the sockets are sorted by created time.
    // The trick is that socket name is consisted of `[NAME].[MILLISECONDS][3_RANDOMIZED_CHARS].sock`, e.g.:
    //    worker.1415603496523pau.sock
    //
    if(sockets && sockets.length > 1) {
      ...
      sockets = sockets.sort(function (sock1, sock2) {
...
function stopOrRestart(action, event, format, target) {
  var emitter = new events.EventEmitter(),
    ppr = action[0].toUpperCase() + action.substr(1) + (action[action.length - 1] == 't' ? '': 'p') + 'ing';

  function sendAction(proc, next) {
    //
    // processing tips...
    //
    forever.log.info(ppr + ' ' + proc.file);
    ...
    function onMessage(data) {
      ...
      var message = data && data.message,
          type    = this.event.pop(),
          wait    = forever.config.get('interval') || 500,
          err;
      //
      //
      // Remark (Tjatse): This message comes from `forever-monitor`, the process is marked as `STOPPED`:
      //    message: Cannot stop process that is not running.
      // We never treat this error as an Exception
      //
      ...
      //
      //
      if (type === 'error' && !/is not running/.test(message)) {
        err = new Error(message);
      }else{
        err = null;
      }
      //
      // When `forever restart/stop(all)` is executed, will only trigger
      // the operation at most once per every wait milliseconds.
      // This is the throttle strategy especially useful for warm-up apps.
      //
      return setTimeout(next, wait, err, data);
    }
  ...
  getAllProcesses(function (err, processes) {
    ...
    if (procs && procs.length > 0) {
      async.mapSeries(procs, sendAction, function (err, results) {
        if (err) {
...
forever.load = function (options) {
  ...
  //
  // Notes: pidPath, logPath, sockPath are readonly,
  //        it leads to a lot of bugs, so must be fixed immediately.
  //
  if (forever.initialized && (options.root || forever.root) === forever.config.get('root')) {
    return;
  }

  forever.config = new nconf.File({ file: path.join(options.root || forever.root, 'config.json') });
  ...
  //
  function setConfig(key, defaultValue){
    // pids, sock, logs are always readonly, and be defined by root.
    forever.config.set(key, !!~['pids', 'sock', 'logs'].indexOf(defaultValue) ?
      path.join(forever.config.get('root'), defaultValue) : (options[key] || forever.config.get(key) || defaultValue));
  }

  setConfig('root', forever.root);
  setConfig('pidPath', 'pids');
  setConfig('sockPath', 'sock');
  setConfig('logPath', 'logs');
  setConfig('loglength', 100);
  setConfig('logstream', false);
  setConfig('columns', []);
  setConfig('interval', 500);
  ...
  ['root', 'pidPath', 'sockPath', 'logPath'].forEach(function(dir){
    tryCreate(forever.config.get(dir));
  });
...
forever.startDaemon = function (script, options) {
  ...
  monitor.on('exit', function (code) {
    forever.out.error('Monitor died unexpectedly with exit code %d', code);
  });
...
forever.logFilePath = function (logFile, uid) {
  return logFile && (logFile[0] === '/' || logFile[1] === ':')
    ? logFile
    : path.join(forever.config.get('logPath'), logFile || (uid || 'forever') + '.log');
};
...
forever.logEvents = function (monitor) {
  ...
  monitor.on('exit:code', function (code, signal) {
    forever.out.error(code != null && !isNaN(code)
