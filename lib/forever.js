...
//
function getSockets(sockPath, pidPath, callback) {
  var sockets;
  ...
  try {
    ...
    if (process.platform == 'win32') {
      try {
        var socketFiles = fs.readdirSync (pidPath);
      } catch (ex) {
        return callback (ex);
      }
      sockets = [];
      return async.each (socketFiles, function (sockname, next) {
        fs.readFile (path.join (pidPath, sockname), function (err, buf) {
          if (err) return next (err);
          if (buf.toString() == '0') return next();
          sockets.push (sockname.slice (0, -4));
          next();
        });
      }, function (ex) {
          callback (ex, sockets);
...
function getAllProcesses(callback) {
  ...
  getSockets(sockPath, pidPath, function (err, sockets) {
    if (err || (sockets && sockets.length === 0)) {
      return callback(err);
    }

    async.map(sockets, getProcess, function (err, processes) {
      callback(undefined, processes.filter(Boolean));
    });
  });
}
...
forever.load = function (options) {
  ...
  options.sockPath  = options.sockPath  || ( process.platform == 'win32' ?
    '\\\\.\\pipe\\forever-sock' : path.join(options.root, 'sock')
  );
