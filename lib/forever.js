...
forever.load = function (options) {
  ...
  function setConfig(key, defaultValue){
    forever.config.set(key, options[key] || forever.config.get(key) ||
    (!!~['pids', 'sock', 'logs'].indexOf(defaultValue) ? path.join(forever.config.get('root'), defaultValue) : defaultValue));
  }
