...
function getSockets(sockPath, callback) {
  ...
  try {
    ...
    //  the trick is socket name is consisted of `[NAME].[MILLISECONDS][3_RANDOMIZED_CHARS].sock`, e.g.:
    //  worker.1415603496523pau.sock
    async.sortBy(sockets, function(soc, callback){
      var tick;
      tick = (tick = soc.match(/\.(\d{13})/)) && tick.length == 2 ? parseFloat(tick[1]) : Date.now();
      callback(null, tick);
...
function stopOrRestart(action, event, format, target) {
  ...
  function sendAction(proc, next) {
    ...
    function onMessage(data) {
      ...
      //
      return setTimeout(next, forever.config.get('interval'), type === 'error' && !/is not running/.test(message)? new Error(message): null, data);
    }
  ...
  getAllProcesses(function (err, processes) {
    ...
    if (procs && procs.length > 0) {
      // restart processes like a waterfall
