...
forever.logEvents = function (monitor) {
  ...
  monitor.on('exit:code', function (code, signal) {
    forever.out.debug("forever.config.store.alert_on_restart: " + forever.config.store.alert_on_restart);
    forever.out.debug("forever.config.store.alert_mode: " + forever.config.store.alert_mode);
    forever.out.debug("forever.config.store.alert_aws_config_accessKeyId: " + forever.config.store.alert_aws_config_accessKeyId);
    forever.out.debug("forever.config.store.alert_aws_config_secretAccessKey: " + forever.config.store.alert_aws_config_secretAccessKey);
    forever.out.debug("forever.config.store.alert_aws_config_region: " + forever.config.store.alert_aws_config_region);
    forever.out.debug("forever.config.store.alert_aws_sns_topicArn: " + forever.config.store.alert_aws_sns_topicArn);
    forever.out.debug("this.logFile: " + this.logFile);
    forever.out.debug("monitor.times: " + monitor.times);
    forever.out.debug("monitor.last_alert: " + monitor.last_alert);
    forever.out.debug("this.args: " + this.args.join(" "));

    if (forever.config.store.alert_on_restart && code && (!monitor.last_alert || (new Date().getTime() - monitor.last_alert) > 60000)) {
      if (forever.config.store.alert_mode == "aws-sns") {

        monitor.last_alert = new Date().getTime();

        if (!monitor.AWS) {
          monitor.AWS = require('aws-sdk');

          monitor.AWS.config.update({
            "accessKeyId": forever.config.store.alert_aws_config_accessKeyId, 
            "secretAccessKey": forever.config.store.alert_aws_config_secretAccessKey, 
            "region": forever.config.store.alert_aws_config_region
          });  

          monitor.sns = new monitor.AWS.SNS();
        }
        
        monitor.sns.publish(
          {TopicArn: forever.config.store.alert_aws_sns_topicArn, Message: "Script failure code: " + code + " " + this.args.join(" "), Subject: "Script failure code: " + code},
          function(err,data){
            if (err){
                forever.log.error("Error sending a message "+err);
            }else{
                forever.log.info("Sent message " + JSON.stringify(data));
            }
          }
        );        
      }
      ...
      forever.log.debug("Not sending alert");
    }
